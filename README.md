...
> **Web is not supported.** `app.json` sets `web.output: "single"` to avoid a Metro crash (`Worker chunk not found for .../expo-sqlite/web/worker.ts`) that otherwise fires on *every* `expo start` — not just `--web` — because Expo Router pre-renders a static web page by default; `"single"` skips that, so plain `npx expo start` runs clean for Expo Go/iOS/Android. If you open the printed `http://localhost:.../` URL in a browser anyway, it'll still fail with `SharedArrayBuffer is not defined`: `expo-sqlite`'s web backend needs the page served with `Cross-Origin-Opener-Policy`/`Cross-Origin-Embedder-Policy` headers, and Expo's dev server (this SDK) serves the HTML document through an internal route handler that doesn't go through Metro's `enhanceMiddleware` hook, so there's no supported way to inject those headers today. Tried it, confirmed it doesn't work (`window.crossOriginIsolated` stays `false`), not worth pursuing further for a mobile-only MVP. Use Expo Go on a phone or an Android/iOS emulator instead.
